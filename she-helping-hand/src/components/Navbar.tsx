  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'glass-nav py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden p-3 -mr-3 text-primary flex items-center justify-center rounded-xl"
            ...
            <div className="px-6 py-10 flex flex-col gap-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-xl font-bold text-gray-800 hover:text-primary active:bg-gray-50 p-4 -m-4 rounded-2xl transition-colors"
              ...
              <button
                ...
                className="w-full text-center bg-primary text-white py-5 rounded-2xl font-bold text-lg shadow-lg active:scale-95 transition-all"
              >
                Donate Now
              </button>
            </div>
